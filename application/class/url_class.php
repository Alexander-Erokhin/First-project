...
class URL {
	//Метод который отдает массив URL 
	public static function routes_url($element = 0){
		//делать очистку от недопустимых символов
		$url_all = trim(urldecode(parse_url($_SERVER["REQUEST_URI"], PHP_URL_PATH)), '/');
		$url_all = preg_replace("/[^a-z0-9\/\-_\s]/u", "", $url_all);
		
		if(!$element){
			return $url_all;
		}
		else{
			$array_url = explode('/', '/'.$url_all);
			if(isset($array_url[$element])) return $array_url[$element];
			else return false;
		}
	}
	
	//Метод который перекидывет на главную страницу
	public static function location_index(){
		return die(header('Location: http://'.$_SERVER['HTTP_HOST']));
	}
