...
<table id="perechen">		
	...
	foreach($data['data_table_all'] as $value){
		if($data['data_filtr_form']['count_table_view'] > 0){
			echo '<tr id="'.$value['ddl_id'].'">';
			echo "<td>".$value['ddl_model_title']."</td>";
			echo "<td>".$value['ddl_type_engine']."</td>";
			echo "<td>".$value['ddl_transmission']."</td>";
			echo "<td>".$value['ddl_volume_cars']."</td>";
			echo "<td>".$value['ddl_year_from']." - ".$value['ddl_year_to']."</td>";
			echo "<td>".$value['ddl_name_engine']."</td>";
			echo "<td>".$value['ddl_price']."</td>";
			echo '<td><span class="kras" onclick="bgPopup('.$value['ddl_id'].')">Запросить</span></td>';
			echo "</tr>";
