...
class Controller_Kpp_dlya_legkovyh extends Controller
{	
	//Страница с выдочей каталог марок
	function action_index()
	{
		$this->model_kpp_dlya_legkovyh = new Model_Kpp_dlya_legkovyh();//Создаем объект для вытаскивания 
		$data['data_bd_view'] = $this->model->get_data_bd("kpp_dlya_legkovyh");
		$data['data_bd_view'] = parent::my_preg_repla($data['data_bd_view']);//Подставляем данные о теле, сате, название, почты..
		$data['name_model_all'] = $this->model_kpp_dlya_legkovyh->get_data_filter_form(URL::routes_url(2));
		$this->view->generate('kpp_dlya_legkovyh_model_view.php', 'template_view.php', $data);//Вызваем Вьевер страницы
	}
	//Страница с выдачи моделей
	function action_table()
	{
		$this->model_kpp_dlya_legkovyh= new Model_Kpp_dlya_legkovyh();//Создаем объект для вытаскивания 
		if(URL::routes_url(3)) $DataArrayAllForFilter = $this->model_kpp_dlya_legkovyh->get_data_filter_form(URL::routes_url(2), URL::routes_url(3));
		else  $DataArrayAllForFilter = $this->model_kpp_dlya_legkovyh->get_data_filter_form(URL::routes_url(2));
		...
		if(URL::routes_url(3)) $data['data_table_all'] = $this->model_kpp_dlya_legkovyh->get_data_table_all(URL::routes_url(2), URL::routes_url(3));
		else  $data['data_table_all'] = $this->model_kpp_dlya_legkovyh->get_data_table_all(URL::routes_url(2));
		...
		foreach($data['data_table_all'] as $key_year_data_table_all => $value_year_data_table_all){
			if($value_year_data_table_all['kdl_year_to'] == 2099){
				$data['data_table_all'][$key_year_data_table_all]['kdl_year_to'] = date('Y');
			}			
		}

		$data['data_bd_view'] = $this->model->get_data_bd("kpp_dlya_legkovyh_mark");
		...
		foreach($data['data_bd_view'][0] as $key => $value){
			$data['data_bd_view'][0][$key] = preg_replace('/{MARK}/', $data['data_table_all'][0]['kdl_mark_title'], $data['data_bd_view'][0][$key]);		
		}
		$this->view->generate('kpp_dlya_legkovyh_view.php', 'template_view.php', $data);//Вызваем Вьевер страницы			
	}
}